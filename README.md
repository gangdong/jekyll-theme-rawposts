...
5. 至此安装完成! 下一步你需要通过配置 _config.yml来定制你自己的网站。
...
#### `客制化设定`
你需要更改`描述`、`标题`和`url`以与个人信息匹配。你还需要将/assets/目录中的`logo`、`默认社交链接`和`默认avatar图像`替换为您自己的图片，并需要将`电子邮件`更改为您要接收的联系人表单查询的电子邮件。
此主题的模板中的默认设置为我的信息，将该部分替换为你自己的信息。
#### `show_excerpts`
...
1. 复制该项目
2. 创建一个分支并对该分支起一个容易辨认的名字并填写好问题或者需求描述，更改记录。
...
### 问题和回答
------
如果你有针对该主题的任何问题，欢迎提交Issue来交流，每个问题我都会认真回复。
<br/><br/>